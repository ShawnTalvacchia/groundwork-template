...
>
> Items are written concisely, and each links its **evidence** — the command output, diff hunk, verify run or screenshot that proves it, never a claim about it. **Evidence obeys the same two rules as the items:** a superseded exhibit is replaced in place, and an exhibit whose item settles is deleted with it unless a live V item or a logged decision still cites it. In a phase run across chats, verdicts land on this doc in the pipeline's lanes (CONTRIBUTING.md → The phase pipeline). Unsure what an item means? **Just ask.**
...
>
> **In a run, V items are deferred, not skipped.** A basic-layer walkthrough carries flow- and feature-level O items only; write the V items as you build and, at that kind's close, move them unwalked to the board's Deepening section. The deepen kind's walkthrough walks them, device-tested. Nothing is walked twice and nothing is dropped.
>
