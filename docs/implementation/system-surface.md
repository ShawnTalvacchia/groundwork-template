...
| Active board(s) | `docs/phases/*.md` (templates excluded) — the board in full under a mode badge, its **stage** pill (brand-filled on the active board), its **status** and its **run**, + progress. The three come from the board's own frontmatter (`status: active \| waiting`, `stage:`, `run:` — CONTRIBUTING § Frontmatter maintenance), read by `getActiveBoards` and never inferred; a board with no `status` reads as active, so boards written before the field existed render as they did. Order is `groupBoards`': by mode — product · system · side · queue-shaping, the Levels line's and the starters' order — the group holding the active board first inside a mode; a **run** is the boards sharing a `run:`, led by the **run board** (the board whose name is the run's name), its members active-first then in the mode's kind sequence (`MODE_KINDS`). On Overview and Work the same groups render as cards (`BoardCards`): a standalone board is one tile, a run is a shelf — the run board full width, the members in a grid — every tile labelled `Active \| Waiting · mode` with the stage as its pill, waiting tiles set back (`.sys-tile-waiting`). Heavy boards derive progress from each `## Workstream` section's Status-cell task rows + legacy checkboxes; light boards (no workstreams) count their `## Items` checkboxes directly, excluding the closing checklist |
| Roadmap | `ROADMAP.md` (Where We Are, the queue table — any mode, Key Considerations, horizon) + `planning/queued/*.md` seeds (`getQueuedSeeds` — **the card's sentence is the ROADMAP row's own Goal cell**; the seed supplies only frontmatter, `mode:` badging each, plus `priority`, `queued` and a note count, and `run:` groups them — rows sharing a run sit together (`groupQueue`: the first keeps the ROADMAP's position, the rest follow it) and each carries the run's name; the queue shelf groups the same way; the whole card links to its seed; the invariant is bidirectional and mode-agnostic — an orphaned seed or a seedless row trips the drift banner). **Sections render in the doc's own order** (`roadmap.sectionOrder`) — reorder the doc and the page follows |
...
- **The surface self-reports** — any failing invariant renders an amber band on every `/system` page, naming the parser, the source doc, and the problem, and prints a `DRIFT [system-surface] …` line at build time (the token leads so a build log is greppable).
