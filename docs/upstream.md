...
# Upstream — this project and its template

> **How this works**
>
> This project started from a template, and the template keeps changing. It publishes each change as a numbered entry in its `CHANGELOG.md`, read where `template:` points. This file is this project's side of that. The steps are `CONTRIBUTING.md` → The upgrade kind.
>
> **`template-entry:`** in the frontmatter is the marker: the last changelog entry this project reviewed. Entries above it are waiting, and taking them is an upgrade. A fresh copy ships with it set to the template's latest entry.
>
