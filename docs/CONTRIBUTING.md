...
- **Concurrency — one open board per mode.** Never two boards of the same mode; the light modes (side, queue-shaping) run alongside the heavy ones freely — a queue-shaping phase edits one row and one seed, never the rules, so the forked-rules risk the limit exists for cannot arise.
...
**Purpose:** A thesis, the change the phase sets out to make. Then the build, then a walkthrough you drive point by point. The deepest ritual of the four, because this is where the product ships.
...
**Purpose:** The docs, the work model, and the dashboard itself. Always done together, never solo.
...
- **You keep opening the same shape of work and it has no name** → name it as a kind. The test is threefold: it recurs, it differs at open or close (its own opening line or its own deliverable), and it lives inside the mode's rituals — tuning a step for the kind is fine (the research kind landing its deliverable in `strategy/research/` is the worked example); needing its own template, badge or band lines is a mode argument, not a kind. Then it's a sentence in the host mode's Purpose and a row in § Session starters.
...
**This is how the two heavy modes — product and system — run work too big for one chat; the side and queue-shaping phases never split.** A heavy phase completes across sessions: its arc — open → build → review → close — runs as three chats, and each chat holds **one capability level, fixed at open**, because switching model or effort mid-chat degrades the work — where the level changes, the chat changes. That change is a **seam**: the outgoing chat commits the board, and the next role begins as a fresh chat, started with its opening line from § Session starters. Your part is the segmenting; everything ritual is built into the phase and runs on its own. Small work skips all of this: the roles collapse into one chat, declared by a board with no Levels line.
...
**A collapsed phase that outgrows small splits at the seam.** The tell is a decision turning into a build. The collapsed chat becomes the planner — the open already did that role's work: seed verified, orient run, scope agreed, board written — so it raises the change with the PO, amends the board with a Levels line, commits, and hands over the executor's opening line; the chats from there carry the split phase's role-led titles.
...
- **Kind** — a named shape of work living inside an existing mode's rituals: the sweep and research (side); product, system and queue-shaping have none. Earned by recurrence, never declared for symmetry — the test lives in § Adjustments, each kind's opening line in § Session starters.
...
- **Boards and seeds** add `mode: product | system | side | queue-shaping` (see "The Work Model").
