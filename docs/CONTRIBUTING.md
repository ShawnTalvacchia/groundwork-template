...
1. **Confirm the walkthrough passed.** The O list is empty and every V point checked, acceptance criteria holding against the running app. **Re-read the remaining items against the running app first** — the phase's own later work is the commonest source of drift, and a concurrent phase touching these surfaces is the other; that phase owed the fix in its own edit, and this is where a miss gets caught.
