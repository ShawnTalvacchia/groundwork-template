...
- **Session** — one chat, holding one capability level fixed at open (§ The phase pipeline). One phase per session is the strong default, not a law: a phase survives a force-ended chat, and a fresh session picking its board back up is continuation, not error-recovery. Names itself when its title is known — `Phase name · mode` collapsed, `kind · Phase name · mode` in a split phase — with each title recorded on the board.
